...
def exp_graph(depth):
    ...
    for d in range(depth):
        ...
        for nodeid in nodeids:
            parent = round(nodeid/2) # intentional integer division
            g.add_edge(str(parent), str(nodeid), 1)
    best_path = [goal]
    while goal > 0:
        goal = round(goal/2) # intentional integer division
    ...
    for nodeid in range(1,2**(depth+1)):
        ...
        while shared_parent not in best_path:
            distance += 1
            shared_parent = round(shared_parent / 2)# intentional integer division
...
#    print(val,hill_climbing_test_6_goal)
