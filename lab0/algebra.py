...
#    raise NotImplementedError
    ...
    if isinstance(expr1,Sum)  and isinstance(expr2,Sum):
        for i in expr1.simplify():
            for j in expr2.simplify():
    ...
    if isinstance(expr1,Sum) and isinstance(expr2,Product):
        ...
        return Sum(result)
    if isinstance(expr2,Sum) and isinstance(expr1,Product):
        for j in expr1.simplify():
            tmp.append(j)
        for i in expr2.simplify():
            result.append(Product(tmp+[i]))
        return Sum(result)
    if isinstance(expr1, Product) and isinstance(expr2,Product):
        for i in expr1.flatten():
            if i ==1:
                continue
            result.append(i)
        for j in expr2.flatten():
            tmp.append(j)
        result.extend(tmp)
